...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (user.profilePictureSecure && user.profilePicture) {
      user.profilePicture = getDecryptedUrl(
        user.profilePicture,
        user.profilePictureSecure
      );
    }
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (interests) {
      ...
      if (Array.isArray(interests)) {
        ...
        return res.status(400).json({
          success: false,
          message:
            "Les centres d'intérêt doivent être fournis sous forme de tableau",
        });
      }
    }
    ...
    if (updatedUser.profilePictureSecure && updatedUser.profilePicture) {
      try {
        updatedUser.profilePicture = getDecryptedUrl(
          updatedUser.profilePicture,
          updatedUser.profilePictureSecure
        );
      } catch (decryptError) {
        console.error(
          "Erreur lors du déchiffrement de l'URL de l'image de profil:",
          decryptError
        );
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'émission de l'événement userProfileUpdated:",
        socketError
      );
    }
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const users = await User.find({ _id: { $ne: req.user._id } }).select(
      "_id username email profilePicture isOnline lastActive profilePictureSecure"
    );
    ...
    const usersWithDecryptedImages = users.map((user) => {
      ...
      if (userObj.profilePictureSecure && userObj.profilePicture) {
        userObj.profilePicture = getDecryptedUrl(
          userObj.profilePicture,
          userObj.profilePictureSecure
        );
      }
      return userObj;
    });
...
exports.getUserById = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select(
        "username email bio profilePicture isOnline lastActive interests gender"
      )
  ...
  try {
    ...
    if (user.blockedUsers.includes(contactId)) {
      return res.status(400).json({
        success: false,
        message:
          "Vous avez bloqué cet utilisateur. Veuillez le débloquer avant de l'ajouter.",
      });
    }
    ...
    const updatedUser = await User.findById(userId).populate(
      "contacts",
      "username profilePicture isOnline lastActive"
    );
