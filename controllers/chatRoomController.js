  ...
  try {
    ...
    if (!["public", "private", "direct"].includes(type)) {
      return res.status(400).json({
        success: false,
        message:
          "Type de salle invalide. Les types valides sont: public, private, direct",
      });
    }
    ...
    if (type === "direct" && (!members || members.length !== 1)) {
      return res.status(400).json({
        success: false,
        message:
          "Les salles directes doivent avoir exactement un membre en plus du créateur",
      });
    }
    ...
    // Émission Socket.io pour synchroniser le profil du groupe en temps réel
    if (req.app.get("io")) {
      req.app.get("io").emit("groupProfileUpdated", {
        groupId: chatRoom._id.toString(),
        groupName: chatRoom.name,
        groupAvatar: chatRoom.avatar,
        groupDescription: chatRoom.description,
      });
    }
...
exports.addMember = async (req, res) => {
  try {
    ...
    await chatRoom.save();
    // Émission Socket.io pour synchroniser le nombre d'utilisateurs et la liste des membres
    if (req.app.get("io")) {
      req.app.get("io").emit("groupUserStatusUpdate", {
        groupId: chatRoom._id.toString(),
        members: chatRoom.members,
        onlineCount: Array.isArray(chatRoom.members)
...
exports.promoteToAdmin = async (req, res) => {
  try {
    ...
    await chatRoom.save();
    // Émission Socket.io pour synchroniser la liste des admins et membres
    if (req.app.get("io")) {
      req.app.get("io").emit("groupUserStatusUpdate", {
        groupId: chatRoom._id.toString(),
        members: chatRoom.members,
  ...
  try {
    ...
    if (chatRoom.creator.toString() === userId.toString()) {
      return res.status(403).json({
        success: false,
        message:
          "Le créateur ne peut pas quitter la salle. Supprimez-la ou transférez la propriété.",
      });
    }
